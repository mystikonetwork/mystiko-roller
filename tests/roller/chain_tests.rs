...
async fn test_run_empty_commitments_success() {
    ...
    mock_handler
        .expect_query_contract_loaded_block()
        .times(3)
        .returning(|_, _| Ok(Some(1000000000)));
    mock_handler.expect_count_commitments().times(3).returning(|_| {
        Ok(QueryResult {
            end_block: 0,
            result: 0,
        })
...
async fn test_chain_rollup_success() {
    ...
    let proof_data = RollupProofData::builder()
        ...
        .build();
    let roller = ChainRoller::new(arc_mock_context).await.unwrap();
    let result = roller.send_rollup_transactions(vec![proof_data]).await;
    assert!(result.is_ok());
}
